...
export function UpdateButton() {
  ...
  const [hideStorageNotification, setHideStorageNotification] =
  ...
  const leftSection = () => {
    return <div className={classes.leftSection}>{changes}</div>;
  };
  ...
  useEffect(() => {
    ...
    if (
      ...
      (calculateTransferDifference(savedTeam, activeTeam) === 0 &&
        savedTeam.length != 0)
    ) {
  ...
  return (
    <div className={classes.container}>
      <Button
        ...
        rightSection={
          <IconArrowRight size={24} className={classes.rightSection} />
        }
        className={
          updatePossible ? classes.updateButton : classes.updateButtonDisabled
        }
        onClick={handleClick}
